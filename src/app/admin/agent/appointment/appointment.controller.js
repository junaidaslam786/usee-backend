...
/**
 * GET /agent/appointment/uncomplete
 * List all appointments
 */
export const listUpcomingAppointments = async (req, res, next) => {
    try {
      const result = await appointmentService.listUpcomingAppointments(req.user, req.query, req.dbInstance);
      if (result?.error && result?.message) {
          return next(createError(400, result.message));
      }
