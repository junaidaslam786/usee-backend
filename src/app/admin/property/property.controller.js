 ...
 */
export const propertyRemovalRequest = async (req, res, next) => {
    try {
        const result = await propertyService.propertyRemovalRequest(req.user.id, req.query, req.dbInstance);
        if (result?.error && result?.message) {
            return next(createError(400, result.message));
        }
...
/**
 * POST /property/removed-property-list
 * Remove property request by agent
 */
export const removedPropertyList = async (req, res, next) => {
    try {
        const result = await propertyService.removedPropertyList(req.user, req.body, req.dbInstance);
        if (result?.error && result?.message) {
            return next(createError(400, result.message));
        }
